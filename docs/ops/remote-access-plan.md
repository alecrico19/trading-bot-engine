...
Free, secure VPN. Both devices get a private IP — phone accesses `http://100.x.x.x:8420` directly.
...
4. Bookmark `http://[PC-tailscale-IP]:8420` on your phone

### Code change needed
HTTP API currently binds to `127.0.0.1` only (local). Needs `--public` flag to bind `0.0.0.0:8420` for Tailscale access.
